...
public class OneLevelRandom implements DecisionMaker {
  public int findNextMove(State state, int user_team) {
    int opponent_team = user_team == 1 ? 2 : 1;
    Game game = new Game();
    Utils utils = new Utils();
    RandomDecision random_decision = new RandomDecision();
    int best_score = -999;
    int best_action = 0;
    ...
    for (int i = 0; i < state.avaliableActions(user_team).size(); i++) {
      ...
      for (int j = 0; j < 5; j++) {
        ...
        int opponent_action_index = random_decision.findNextMove(state_copy, opponent_team);
        ...
        while (!state_copy.terminalState()) {
          int p1_move = random_decision.findNextMove(state_copy, 1);
          int p2_move = random_decision.findNextMove(state_copy, 2);
